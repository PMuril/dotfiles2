...
install_zsh_vi_mode () {
    printf "installing zsh-vi-mode\n"
    wget https://github.com/jeffreytse/zsh-vi-mode/archive/refs/heads/master.zip \
    && unzip master.zip \
    && rsync -av --exclude=".git" "./zsh-vi-mode" "${HOME}/.config/zsh/plugins/zsh-vi-mode-master" \
    && rm -r master.zip* zsh-vi-mode-master
}

install_zsh_z () {
    printf "installing zsh-z\n"
    wget https://github.com/agkozak/zsh-z/archive/refs/heads/master.zip \
    && unzip master.zip \
    && rsync -av --exclude=".git" "./zsh-z-master/" "${HOME}/.config/zsh/plugins/zsh-z" \
    && rm -e master.zip* zsh-z-master
}
